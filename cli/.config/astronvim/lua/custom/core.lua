    ...
    opts = {
      post_restore_cmds = {
        function()
          ...
          if not close_timer then return end
          ...
          close_timer:start(
            ...
            vim.schedule_wrap(function()
              for i, buf in ipairs(vim.fn.getbufinfo { buflisted = 1 }) do
                if buf.name == "" or buf.name == "[No Name]" then Snacks.bufdelete(buf.bufnr) end
              end
      ...
      vim.api.nvim_create_autocmd({ "InsertEnter", "InsertLeave" }, {
        callback = function(args)
          if args.event == "InsertEnter" then
            ...
            if not c.is_buffer_attached() then c.attach_to_buffer() end
          end
        end,
      })
    end,
