...
--
local function maphelper2(key, c, desc, remap) maphelper(key, c, c, c, desc, remap) end
...
map({ "n", "v" }, "D", '"_D')
map({ "n", "v" }, "c", '"_c')
map("n", "!", function() funcs.definition_in_float() end)
map({ "n", "v", "i" }, "<A-g>", function() funcs.cycle_through_marks() end)

map({ "n", "v", "i" }, "<A-u>", function() funcs.cycle_through_marks(true) end)
...
if vim.g.neovide then
  ...
  maphelper2("<C-+>", function() vim.g.neovide_scale_factor = vim.g.neovide_scale_factor * 1.1 end)
  maphelper2("<C-->", function() vim.g.neovide_scale_factor = vim.g.neovide_scale_factor * 1 / 1.1 end)
  maphelper2("<C-0>", function() vim.g.neovide_scale_factor = 1 end)
end
...
map("n", "<leader>fh", function() vim.cmd "DiffviewFileHistory % --no-merges" end, { desc = "File history" })
...
-- -- Telescope
-- --
maphelper2("<C-.>", function() funcs.code_actions() end, "Code actions")
maphelper2("<C-Space>", function() vim.lsp.buf.hover() end, "Code actions")
...
maphelper2("<C-s>", function() funcs.smart_save() end, "Smart save")
maphelper2("<C-r>", function() Snacks.picker.recent { title = "Recent files" } end, "Recent files")

maphelper2("<C-d>", function() Snacks.picker.lsp_symbols { title = "LSP Symbols" } end, "LSP symbols")
...
map("n", "<esc>", function() funcs:smart_close() end, { buffer = false, noremap = true })

map("n", "<C-\\>", function() vim.cmd "terminal" end, { desc = "Open terminal" })
...
map("n", "<leader>fi", function() funcs.show_file_info_popup() end, { desc = "File info" })
